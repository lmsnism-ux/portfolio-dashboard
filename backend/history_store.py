...
같은 날짜에 다시 호출되면 최신값으로 덮어쓴다.

스냅샷 시점 정규화 (main.py 스케줄러 참조):
- 사용자 호출 시각의 ad-hoc 적재는 일중 비균질 시점이 되므로,
  KST 16:30 (한국장 마감 직후) 과 06:30 (미국장 마감 직후) cron
  backfill이 한국·미국 종가가 반영된 일관된 스냅샷을 마지막에 덮어쓴다.
- 그 결과 모든 과거 스냅샷은 '시장 마감 후' 동일 시점 기준이 된다.
"""
