...
export default function DonutChart({ data, title, hideAssets = false, compact = false }: Props) {
  ...
  return (
    <div className="bg-toss-card rounded-[var(--radius-toss-lg)] border border-toss-border shadow-[var(--shadow-toss-card)] p-4">
      <h3 className={`font-semibold text-toss-text-secondary mb-1 ${compact ? 'text-xs' : 'text-sm'}`}>{title}</h3>
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-4 text-center">
          {active ? (
            <>
              <span className="text-[10px] text-toss-text-tertiary truncate max-w-[90px]">
                {active.name}
              </span>
              <span className="num text-sm font-bold text-toss-text-primary truncate max-w-[90px]">
            ...
            <>
              ...
              <span className={`num font-bold text-toss-text-primary ${compact ? 'text-sm' : 'text-base'}`}>
      ...
      <ul className="mt-2 space-y-1.5">
        ...
        {data.length > maxLegend && (
          <li className="text-[11px] text-toss-text-tertiary text-center pt-0.5">
            +{data.length - maxLegend}개 더
          </li>
        )}
      </ul>
    </div>
